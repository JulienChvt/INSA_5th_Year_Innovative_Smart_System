...
public class HueSdkListener implements PHSDKListener {
	...
	public void onCacheUpdated(List cacheNotificationsList, PHBridge bridge) {
		...
		for (PHLight light : cache.getAllLights()){
			if (cacheNotificationsList.contains(PHMessageType.LIGHTS_CACHE_UPDATED)) {
				...
				monitor.pushState(light.getIdentifier(), light.getLastKnownLightState().isOn().toString(), HueUtil.getColor(light.getLastKnownLightState().getHue()), light.getLastKnownLightState().getHue() );
			}
		}
	...
	public void onBridgeConnected(PHBridge b, String username) {
		...
		// TODO create resources in OM2M for each light connected to the bridge
		Monitor monitor = new Monitor();
		PHBridgeResourcesCache cache = b.getResourceCache();
		for (PHLight light : cache.getAllLights()){
			monitor.createResources(light.getIdentifier());
		}
	}
